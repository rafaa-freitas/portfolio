...
function TechCard({ imageSrc, imageAlt, title, techLink }: TechCardProps) {
  return (
    <a href={techLink} target="_blank">
      <div className="relative flex items-end justify-center row-span-1 col-span-1 rounded-2xl border border-white/20 w-48 h-48 p-4 overflow-hidden group cursor-pointer">
        <div className="absolute w-full h-[50%] bottom-0 bg-linear-to-t from-[#171F2C] to-[#17202e00] z-10"></div>
        ...
        <h5 className="text-white-text z-10">{title}</h5>
      </div>
    </a>
  );
}
